...
export const siteConfig: SiteConfig = {
  ...
  colors: {
    // Strict yellow/black brand — every color token on the site is either a
    // shade of yellow/gold or a near-black neutral. No red, green, tan/cream,
    // or any other hue appears anywhere (glows, overlays, badges, buttons).
    // accent is the saturated, unmistakably-yellow brand color used for all
    // interactive/emphasis text (nav links, prices, labels, buttons).
    accent: '#FFD300',
    // Lighter tint of accent, used for large glows/highlights where full
    // saturation would be too loud.
    primary: '#FFDE59',
    // Darker tint of accent, used for warm photo-overlay tints and
    // alternating-row backgrounds (e.g. the signature-dish photo gradient
    // on the home page) — replaces the template's wine-red default so
    // those overlays read as gold/black, never red.
    primaryDark: '#8A6400',
    background: '#12100D',
  },
  contact: {
